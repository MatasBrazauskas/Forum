...
import { THREADS_QUERY_KEY } from "../Utils/queryConsts";
...
function AddNewTheadComponent({topicsName}: {topicsName: string}){
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await addNewThread(data);
        queryClient.invalidateQueries({ queryKey: [THREADS_QUERY_KEY, topicsName]});
    }
    ...
    return (
        <div>
            ...
            {open && <form onSubmit={(e) => handleSubmit(e)} className='addTopic'>
                <input type='text' className='w-140' maxLength={THREADS_TITLE_LENGTH} ref={title} placeholder="Enter Threads Title"/>
                <input type='text' className='w-140' maxLength={THREADS_CONTENT_LENGTH} ref={content} placeholder="Enter Content"/>
